    ...
    <el-form class="formContainer" :rules="rules" :model="formData" ref="formRef" label-width="140px">
      ...
      <el-form-item label="类型:" prop="category">
        <el-radio-group v-model="formData.category">
          <el-radio :label="item.label" v-for="item in dialogCategory" :key="item.value"></el-radio>
        </el-radio-group>
      </el-form-item>
...
import { getDialogCategory } from '@/filters/transform'
export default {
  name: '',
  data () {
    return {
      dialogCategory: getDialogCategory(),
      // 验证规则
      rules: {
    ...
    saveForm () {
      this.$refs.formRef.validate(valid => {
        if (valid) {
          ...
          this.$router.push({ name: 'HomeDialogListIndex' })
        } else {
