...
<template>
  <div class="goodsFloor">
    ...
    <el-form class="formContainer" :rules="rules" :model="formData" ref="formRef" label-width="140px">
      ...
      <!-- text输入框 -->
      <el-form-item label="首页商品数量:" prop="goodsNum">
        <el-input-number v-model="formData.goodsNum" controls-position="right" @change="handleGoodsNumChange" :min="4" :max="12" placeholder="4~12以内数字"></el-input-number>
        <!-- <el-input v-model="ruleForm.goodsNum" clearable placeholder="4~12以内"></el-input> -->
      </el-form-item>
  ...
  data () {
    return {
      ...
      rules: {
        ...
        goodsNum: [{ required: true, message: '必须为4~12以内的数字', trigger: 'blur' }],
    ...
    saveForm () {
      this.$refs.formRef.validate(valid => {
        if (valid) {
          ...
          this.$router.push({ name: 'GoodsFloorListIndex' })
        } else {
