...
import categoryModal from '@/views/message/components/help/categoryModal'
    ...
    return {
      formOptions: [
        ...
        },
        {
          ...
          props: {
            lazy: true,
            checkStrictly: true, // 单选可选任一级菜单
            emitPath: false, // 不取数组，只取该节点的值
            lazyLoad (node, resolve) {
              ...
              setTimeout(() => {
                const nodes = Array.from({ length: level + 1 })
                  .map((item, index) => ({
                    value: Math.pow(10, level) + index,
                    label: `选项${Math.pow(10, level) + index}`,
                    leaf: level >= 2
                  }))
                resolve(nodes)
              }, 1000)
